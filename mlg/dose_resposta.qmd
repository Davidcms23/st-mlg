...
Na componente sistemática, o tratamento da dose zero (grupo controle) exige adaptação matemática, visto que $\log(0)$ é indefinido. O comportamento natural sem toxina resulta em uma divisão aproximada da amostra entre os recipientes, inviabilizando o ajuste de uma regressão ordinária a $\log(Dose)$. Para contornar esta limitação e avaliar a sensibilidade do modelo, serão testadas duas abordagens estruturais:

 - **Deslocamento escalar**: Adição de constantes arbitrárias à variável explicativa para viabilizar a transformação logarítmica em todo o conjunto de dado, testando os preditores $\log(Dose + 1)$ e $\log(Dose + 0,1)$.
